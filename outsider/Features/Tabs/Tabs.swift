...
struct Tabs {
  ...
  var body: some Reducer<State, Action> {
    Reduce { state, action in
      ...
      case .subscribeStories:
        return .run { [following = state.currentUser.following, currentUser = state.currentUser] send in
      ...
      case .initialize:
        return .merge(
          ...
          .run { send in
            await send(.currentProfile(.profile(.fetchPosts)))
          },
          .run { send in
      ...
      case .home(.posts(.element(id: _, action: .didMute(.success(let uuid))))):        
        let userUuid = state.currentUser.uuid
        let muted = PostMutedModel(
          post_uuid: uuid,
          user_uuid: userUuid
        )
        state.currentUser.mutedPosts.append(muted)
        state.home.currentUser.mutedPosts.append(muted)
        state.home.stories.currentUser.mutedPosts.append(muted)
        state.currentProfile.profile.currentUser.mutedPosts.append(muted)
        state.messages.currentUser.mutedPosts.append(muted)
        state.camera.cameraSend.currentUser.mutedPosts.append(muted)
        state.explore.currentUser.mutedPosts.append(muted)
        return .none
      ...
      case .home(.posts(.element(id: _, action: .didReport(.success(let uuid))))):
        state.bannerData = BannerModifier.BannerData(
          title: "Reported",
          detail: "Post \(uuid.uuidString) reported.",
          type: .Info
        )
        state.showBanner = true
        return .none
